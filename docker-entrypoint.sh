...
# ── 모델 ──
#
# 전사 모델 671MB + 화자 분리 모델 34.3MB. 이미지에 굽지 않고 볼륨에 두고
# 여기서 챙긴다 — 이미 있으면 스크립트가 곧바로 끝난다 (파일 검사 몇 번).
#
# **실패해도 앱은 뜬다.** 여기서 멈추면 네트워크가 잠깐 안 될 때 앱 전체가
# 안 뜬다. 모델이 없으면 전사만 실패하고, 그 실패 메시지가 무엇이 없는지
# 그대로 말해 준다 — 화면은 멀쩡히 열린다.
#
# 스크립트가 0 이 아닌 값으로 끝나는 것은 **전사 모델**이 없을 때뿐이다.
# 분리 모델이 없는 것은 스크립트가 제 안에서 말하고 넘어간다 — 전사문이
# 화자 분리보다 먼저이고, 분리가 없으면 화자 구분만 못 한다.
# 그래서 아래 실패 문구도 전사 이야기만 한다.
if [ "${AUTO_FETCH_MODEL:-1}" = "1" ]; then
  # 스택이 자리를 물려 줬으면 그쪽이 이긴다 (`scripts/fetch-model.mjs`).
  # 여기서 MODEL_DIR 만 찍으면 실제로 쓰는 자리와 다른 말을 하게 된다.
  echo "[voicebento] 모델을 확인합니다 (전사 ${ASR_MODEL_DIR:-${MODEL_DIR:-/app/data/models}} · 분리 ${DIAR_MODEL_DIR:-${MODEL_DIR:-/app/data/models}})."
  node /app/scripts/fetch-model.mjs || \
    echo "[voicebento] 전사 모델을 준비하지 못했습니다. 전사만 실패하고 앱은 그대로 뜹니다." >&2
fi
